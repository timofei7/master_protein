...
class MasterSearch(Wizard):
    ...
    def update(self):
        ...
        # Open redesign popup (Trevor, Ahsan, Anish)
        elif self.makeLogo == 4 and self.res_live != True:
            print "opening redesign app"
            self.res_app = RedesignApp(self.app)
            self.res_live = True
    ...
    def get_panel(self):
        ...
        print "testing get_panel"
        return [[2, 'Search Menu','cmd.get_wizard().logo_helper(3)'], [2, 'Redesign Module','cmd.get_wizard().logo_helper(4)'], [2, 'Exit', 'cmd.get_wizard().logo_helper(2); cmd.set_wizard()']]
