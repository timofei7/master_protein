...
class RedesignThread(ServerThread):
    """
    search thread allows the ui to remain responsive while this sends off the search request and waits
    """

    def __init__(self, wiz, pdbstrs, protstr, serverURL, thecmd, dictionary):
        ...
        data = [
            ("protein", (pycurl.FORM_BUFFER, 'something', pycurl.FORM_BUFFERPTR, protstr))
            ("selection", (pycurl.FORM_BUFFER, 'sele.pdb', pycurl.FORM_BUFFERPTR, pdbstrs)),
            ("bbRMSD", "on")
        ]

        super(RedesignThread, self).__init__(serverURL + '/api/redesign', data)
